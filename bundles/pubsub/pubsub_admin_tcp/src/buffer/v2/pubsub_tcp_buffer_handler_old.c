...
pubsub_tcpBufferHandler_pt pubsub_tcpBufferHandler_create(unsigned int maxNofBuffers) {
  ...
  if (handle != NULL) {
    handle->maxNrLists = maxNofBuffers;
...
//
int largeUdp_sendmsg(largeUdp_pt handle, int fd, struct iovec *largeMsg_iovec, int len, int flags,
                     struct sockaddr_in *dest_addr, size_t addrlen) {
  int n;
  int result = 0;
  msg_part_header_t header;
  ...
  for (n = 0; n < nr_buffers; n++) {
    ...
    header.part_msg_size = (((header.total_msg_size - n * MAX_PART_SIZE) > MAX_PART_SIZE) ? MAX_PART_SIZE : (
      header.total_msg_size - n * MAX_PART_SIZE));
    ...
    while (remainingData > 0) {
      int partLen = ((largeMsg_iovec[recvPart].iov_len - remainingOffset) <= remainingData ? (
        largeMsg_iovec[recvPart].iov_len - remainingOffset) : remainingData);
...
// Write large data to UDP. This function splits the data in chunks and sends these chunks with a header over UDP.
//
int largeUdp_sendto(largeUdp_pt handle, int fd, void *buf, size_t count, int flags, struct sockaddr_in *dest_addr,
                    size_t addrlen) {
  int n;
...
//
int largeUdp_read(pubsub_tcpBufferHandler_pt handle, unsigned int index, void **buffer, unsigned int size) {
  int result = 0;
  pthread_mutex_lock(&handle->dbLock);
