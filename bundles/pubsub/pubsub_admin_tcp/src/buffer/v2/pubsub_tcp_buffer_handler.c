...
pubsub_tcpBufferHandler_pt pubsub_tcpBufferHandler_create() {
  ...
  if (handle != NULL) {
    ...
    handle->url_map = hashMap_create(NULL, NULL, NULL, NULL);
...
int pubsub_tcpBufferHandler_connect(pubsub_tcpBufferHandler_pt handle, char *url, celix_thread_mutex_t* mutex,  bool *connected,  bool *all_connected) {
  ...
  if (rc >= 0) {
    ...
    if (rc < 0) {
      ...
      entry->all_connected = all_connected;
      if (entry->connected) *entry->connected = false;
    }
  }
  if (rc >= 0) {
    struct epoll_event event;
  ...
  if ((rc >= 0) && (entry)) {
    if (entry->connected) *entry->connected = true;
...
int pubsub_tcpBufferHandler_close_connection(pubsub_tcpBufferHandler_pt handle, char *url) {
  ...
  if (handle != NULL) {
    ...
    if (entry->fd >= 0) {
      ...
      if (entry->connected)     *entry->connected     = false;
      if (entry->all_connected) *entry->all_connected = false;
...
#ifdef NOF
int pubsub_tcpBufferHandler_bind(pubsub_tcpBufferHandler_pt handle, char* url) {
  pubsub_tcp_url_t url_info;
  pubsub_tcpBufferHandler_set_url_info(url, &url_info);
  if (handle->fd >= 0) {
    struct sockaddr_in addr; // connector's address information
    int rc = pubsub_tcpBufferHandler_set_in_addr(url_info.hostname, url_info.portnr, &addr);
    ...
    if (rc == 0) rc = listen(handle->fd, SOMAXCONN);
    ...
    if (rc >= 0 ) {
      ...
      if (handle->bypassIpFragmentation == false) {
        rc = getsockopt(handle->fd, IPPROTO_IP, IP_MTU, &handle->mtuSize, &len);
...
pubsub_tcpBufferHandler_read(pubsub_tcpBufferHandler_pt handle, unsigned int index, pubsub_tcp_msg_header_t **header,
  ...
  if (item) {
    if (handle->bypassHeader) {
      ...
      *buffer = &item->buffer[sizeof(pubsub_tcp_msg_header_t)];
    }
  } else {
    result = -1;
  }
  ...
  if (handle->efd >= 0) {
    ...
    for (i = 0; i < nof_events; i++) {
      if ((handle->fd >= 0) && (events[i].data.fd == handle->fd)) {
        ...
        rc = accept(handle->fd, &remote, &len);
        if (rc == -1) {
